...
const CIRCUITS_TO_MERGE = EXAMPlE_MODE ? 10 : 1000;
...
function solve() {
  ...
  for (let distance of sorted.slice(0, CIRCUITS_TO_MERGE)) {
  ...
  const threeBiggestCircuits = circuits.sort((a, b) => b.size - a.size).slice(0, 3);
  const result = threeBiggestCircuits.reduce((acc, circuit) => circuit.size * acc, 1)
